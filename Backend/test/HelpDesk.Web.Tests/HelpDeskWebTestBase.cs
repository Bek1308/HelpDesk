...
public abstract class HelpDeskWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
{
    protected static readonly Lazy<string> ContentRootFolder;

    static HelpDeskWebTestBase()
    {
    ...
    protected override IWebHostBuilder CreateWebHostBuilder()
    {
        return base
            ...
            .UseSetting(WebHostDefaults.ApplicationKey, typeof(HelpDeskWebMvcModule).Assembly.FullName);
    }
    ...
    {
        using (var context = IocManager.Resolve<HelpDeskDbContext>())
        {
            action(context);
            context.SaveChanges();
        }
    ...
    protected async Task UsingDbContextAsync(Func<HelpDeskDbContext, Task> action)
    {
        using (var context = IocManager.Resolve<HelpDeskDbContext>())
        {
    ...
    protected async Task<T> UsingDbContextAsync<T>(Func<HelpDeskDbContext, Task<T>> func)
    {
        T result;
