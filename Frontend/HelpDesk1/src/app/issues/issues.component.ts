...
export class IssuesComponent extends AppComponentBase implements OnInit, AfterViewInit, OnDestroy {
  records: IssuesDto[] = [];
  isLoading = false;
  ...
  availableColumns: { key: string; label: string; selected: boolean }[] = [
    ...
    // CallCenterData columns
    { key: 'callcenterdata.subcategoryname', label: 'CallCenterSubCategoryName', selected: true },
    { key: 'callcenterdata.servicename', label: 'CallCenterServiceName', selected: true },
    { key: 'callcenterdata.subscriber', label: 'Subscriber', selected: false },
    ...
    { key: 'callcenterdata.terminalnumber', label: 'CallCenterTerminalNumber', selected: false },
    { key: 'callcenterdata.sum', label: 'Sum', selected: false },
    { key: 'callcenterdata.cancelledsum', label: 'CancelledSum', selected: false },
    // RepairData columns
    { key: 'repairdata.agentfullname', label: 'AgentFullName', selected: false },
    { key: 'repairdata.agentnumber', label: 'RepairAgentNumber', selected: false },
    { key: 'repairdata.equipment', label: 'Equipment', selected: false },
    { key: 'repairdata.serialnumber', label: 'SerialNumber', selected: false },
    { key: 'repairdata.issuedescription', label: 'RepairIssueDescription', selected: false },
    ...
    // TechDepartmentData columns
    { key: 'techdepartmentdata.terminalnumber', label: 'TechTerminalNumber', selected: false },
    { key: 'techdepartmentdata.terminalname', label: 'TerminalName', selected: false },
    { key: 'techdepartmentdata.agentid', label: 'AgentId', selected: false },
    { key: 'techdepartmentdata.agentnumber', label: 'TechAgentNumber', selected: false },
    { key: 'techdepartmentdata.issuedescription', label: 'TechIssueDescription', selected: false },
    { key: 'techdepartmentdata.issuegroupname', label: 'IssueGroupName', selected: false },
    { key: 'techdepartmentdata.terminallocation', label: 'TerminalLocation', selected: false },
  ...
  delete(recordId: number): void {
    this.message.confirm(
      this.l('AreYouSureWantToDelete', 'this issue'),
      this.l('AreYouSure'),
      (result: boolean) => {
        if (result) {
          this._issuesService.delete(recordId).subscribe({
            next: () => {
            ...
            error: (error) => {
              ...
              this.notify.error(this.l('FailedToDeleteIssue'));
            },
          });
        }
      }
  ...
  createIssue(): void {
    this.showCreateOrEditIssueDialog();
  }

  editIssue(recordId: number): void {
    this.showCreateOrEditIssueDialog(recordId);
    this.dropdownOpenId = null;
    this.dropdownPosition = null;
    this.cd.markForCheck();
  }
  ...
  viewIssue(recordId: number): void {
    ...
    // this.dropdownPosition = null;
    // this.cd.markForCheck();
    this.router.navigate(['/app/issue_workspace', recordId]);
  }

  showCreateOrEditIssueDialog(id?: number): void {
  ...
  saveColumnPreferences(): void {
    localStorage.setItem('issuesColumns', JSON.stringify(this.availableColumns));
    this.selectedColumns = this.availableColumns.filter(col => col.selected);
  }

  loadColumnPreferences(): void {
    const saved = localStorage.getItem('issuesColumns');
    if (saved) {
  ...
  saveFilterPreferences(): void {
    ...
    const cacheKey = `filters_Issues_${userId}`;
