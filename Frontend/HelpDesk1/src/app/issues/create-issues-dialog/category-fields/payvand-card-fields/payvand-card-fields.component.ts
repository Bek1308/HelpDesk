...
import { CreateIssuesDto, PayvandCardIssueCommandDto } from '@shared/api-services/issues/model/issues-dto.model';
...
@Component({
  selector: 'app-payvand-card-fields',
  templateUrl: './payvand-card-fields.component.html',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class PayvandCardFieldsComponent extends AppComponentBase implements OnInit {
  ...
  loadData(): void {
    ...
    if (this.issue.payvandCardData) {
      this.selectedSubCategory = this.subCategories.find(sc => sc.id === this.issue.payvandCardData?.subCategoryId) || null;
    }
  }
