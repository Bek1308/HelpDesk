...
import { CreateIssuesDto, PayvandWalletIssueCommandDto } from '@shared/api-services/issues/model/issues-dto.model';
...
@Component({
  selector: 'app-payvand-wallet-fields',
  templateUrl: './payvand-wallet-fields.component.html',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class PayvandWalletFieldsComponent extends AppComponentBase implements OnInit {
  ...
  ngOnInit(): void {
    this.loadData();
    if (!this.issue.payvandWalletData) {
  ...
  loadData(): void {
    ...
    if (this.issue.payvandWalletData) {
      this.selectedSubCategory = this.subCategories.find(sc => sc.id === this.issue.payvandWalletData?.subCategoryId) || null;
      this.selectedService = this.services.find(s => s.id === this.issue.payvandWalletData?.serviceId) || null;
    }
  }
