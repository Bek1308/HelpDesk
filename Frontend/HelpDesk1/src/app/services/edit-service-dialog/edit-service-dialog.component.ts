...
export class EditServiceDialogComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        injector: Injector,
        public _servicesService: ServicesServiceProxy,
    ...
    ngOnInit(): void {
        ...
        this._servicesService.getForEdit(this.id).subscribe({
            next: (result: ServiceDto) => {
                this.service = result;
                this.originalKey = this.service.name;
                this._languagesTextService.getByKey(this.originalKey, '').subscribe({
                    next: (texts: LanguagesTextDto[]) => {
                        if (texts && Array.isArray(texts) && texts.length > 0) {
                            ...
                            this.localizedTitles['default'] = this.service.name;
                        }
            ...
            error: () => {
                this.notify.error(this.l('FailedToLoadService'));
                this.cd.detectChanges();
            }
        });
    }
    ...
    save(): void {
        ...
        serviceData.init({
            ...this.service,
            name: this.normalizeKey(this.localizedTitles['en'] || this.localizedTitles['default'] || '')
        });

        this._servicesService.update(serviceData).subscribe({
            next: () => {
