...
import { CreateServiceDto } from '@shared/api-services/services/model/service-dto.model';
...
export class CreateServiceDialogComponent extends AppComponentBase implements OnInit {
    @ViewChild('createServiceForm') createServiceForm: NgForm;
    ...
    save(): void {
        ...
        const serviceData = new CreateServiceDto();
        serviceData.init({
            ...this.service,
            name: this.normalizeKey(this.localizedTitles['en'] || this.localizedTitles['default'] || '')
        });

        this._servicesService.create(serviceData).subscribe(
            () => {
