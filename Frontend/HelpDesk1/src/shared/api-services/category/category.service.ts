...
import { CategoryDto, CategoryDtoPagedResultDto, CreateCategoryDto } from './model/category-dto.model';
import { API_BASE_URL } from '../../service-proxies/service-proxies';
import { ICategoryDto } from './interface/category-dto.interface';
 ...
 * @return OK
 */
getForEdit(id: number): Observable<CategoryDto> {
...
protected processGetForEdit(response: HttpResponseBase): Observable<CategoryDto> {
    const status = response.status;
    const responseBlob =
        response instanceof HttpResponse ? response.body :
        (response as any).error instanceof Blob ? (response as any).error : undefined;
    ...
    if (status === 200) {
        return blobToText(responseBlob).pipe(
            _observableMergeMap((_responseText: string) => {
                ...
                const result200 = CategoryDto.fromJS(resultData200);
                return _observableOf(result200);
            })
        );
    } else if (status !== 200 && status !== 204) {
